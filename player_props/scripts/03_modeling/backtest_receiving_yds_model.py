...
def main():
    ...
    # TEMP TEST: receiving yards v1 filter
    picks = picks[
        (picks["recommended_ev_percent"] >= 2)
        & (picks["recommended_ev_percent"] <= 10)
    ].copy()

    print(f"[filter] EV 2-10 picks={len(picks):,}")

    picks["player_norm"] = picks["player"].apply(norm_player)
    hist["player_norm"] = hist["player"].apply(norm_player)

    actual_col = None
    for c in ["actual", "actual_receiving_yards", "receiving_yards"]:
        if c in hist.columns:
            actual_col = c
            break

    if actual_col is None:
        raise RuntimeError(f"No actual receiving yards column found. Columns={hist.columns.tolist()}")
    ...
    df.to_csv(OUT_DIR / "reception_yds_backtest_rows.csv", index=False)
    summary.to_csv(OUT_DIR / "reception_yds_backtest_summary.csv", index=False)
    by_edge.to_csv(OUT_DIR / "reception_yds_backtest_by_edge_bucket.csv", index=False)
    by_ev.to_csv(OUT_DIR / "reception_yds_backtest_by_ev_bucket.csv", index=False)
    by_side.to_csv(OUT_DIR / "reception_yds_backtest_by_side.csv", index=False)
