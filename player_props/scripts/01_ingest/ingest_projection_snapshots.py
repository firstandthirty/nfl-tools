...
from projection_adapters.fourforfour import (
    build_sanity_warnings as build_fourforfour_sanity_warnings,
    build_validation_report as build_fourforfour_validation_report,
    transform_fourforfour_snapshot,
)
...
def ingest_fourforfour_snapshot(raw_file: Path | str, *, season: int | str, week: int | str, output_root: Path | str, manifest_path: Path | str | None = None, weekly_output_path: Path | str | None = None, skip_registry_update: bool = False) -> dict:
    raw_path = Path(raw_file)
    metadata = parse_snapshot_metadata(raw_path, source="4for4", season=season, week=week)
    output_paths = build_output_paths(output_root, source="4for4", season=season, week=week, raw_file=raw_path)

    manifest_path = Path(manifest_path) if manifest_path is not None else output_paths["output_dir"] / "ingested_snapshots.csv"
    manifest_path.parent.mkdir(parents=True, exist_ok=True)
    manifest_columns = ["source", "season", "week", "raw_file", "captured_at", "captured_at_source"]
    manifest_df = pd.DataFrame(columns=manifest_columns)
    if manifest_path.exists() and manifest_path.stat().st_size > 0:
        manifest_df = pd.read_csv(manifest_path)
    ...
    raw_file_key = str(raw_path.resolve())
    raw_df = pd.read_csv(raw_path)
    rows, rejected, skipped_fields = transform_fourforfour_snapshot(raw_df, raw_file=raw_path, metadata=metadata)
    if not manifest_df.empty and raw_file_key in manifest_df.get("raw_file", pd.Series(dtype=str)).astype(str).tolist():
        return {"skipped": True, "rows_written": len(rows), "skipped_fields": len(skipped_fields), "output_paths": {"long": str(output_paths["long_path"]), "validation": str(output_paths["validation_path"]), "rejected": str(output_paths["rejected_path"])}}

    warnings = build_fourforfour_sanity_warnings(rows)
    if metadata.captured_at_source != "filename":
        warnings.append(f"capture_time_fallback={metadata.captured_at_source}")
    ...
    columns = [
        ...
        "source_player_id", "source_row_number", "source_column",
    ]
    rejected_columns = ["raw_file", "source_row_number", "player", "position", "market", "source_column", "reason", "value"]
    long_df = pd.DataFrame(rows)
    for column in columns:
        if column not in long_df.columns:
            long_df[column] = pd.NA
    validation_df = build_fourforfour_validation_report(raw_df, rows, rejected, skipped_fields, metadata, warnings)
    ...
    if not skip_registry_update:
        try:
            registry_result = build_projection_registry(project_root=COMMON_PROJECT_ROOT, output_root=COMMON_PROJECT_ROOT, source="4for4", season=season, week=week)
        except Exception as exc:
            registry_result = {"registry_error": str(exc)}
...
def main() -> None:
    ...
    if source == "fantasypros":
        ...
        for raw_file in raw_files:
            result = ingest_fourforfour_snapshot(raw_file, season=season, week=week, output_root=COMMON_PROJECT_ROOT, skip_registry_update=args.skip_registry_update)
            if result.get("skipped"):
