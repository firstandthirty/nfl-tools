...
def fetch_event_odds(event):
    ...
    params = {
        "apiKey": API_KEY,
        "regions": "us",
        "markets": f"{MARKET},spreads,totals,h2h",
    ...
    # Debug: log request URL/params for first event
    if not DEBUG_EVENT_LOGGED:
        DEBUG_EVENT_LOGGED = True
        print(f"[prop_request] url={url}")
        print(f"[prop_request] markets={params.get('markets')} bookmakers={params.get('bookmakers')} regions={params.get('regions')} date={params.get('date')}")

    data, cached = get_json(url, params, raw_file)
    
    # Check if response includes game context markets
    ...
    print(f"[prop_response] market_keys={sorted(response_markets)}")
    
    # If spreads/totals/h2h not in response, make separate request for game context
    if not {"spreads", "totals", "h2h"} & response_markets:
        print(f"[prop_response] game context markets missing, fetching separately")
        context_data = fetch_event_odds_context(event_id, snapshot_time)
        if context_data:
            data = merge_context_into_props(data, context_data)
...
def flatten_event_odds(payload):
    ...
    # Collect game context from all bookmakers, preferring FanDuel
    # First pass: look for all game context markets across all bookmakers
    for book in event.get("bookmakers", []):
        bkey = book.get("key")
        ...
        for market in book.get("markets", []):
            ...
            # spreads
            if mkey == "spreads" and not game_ctx["spreads"]:
                for out in market.get("outcomes", []):
                    name = out.get("name")
                    pt = out.get("point")
                    if name is not None and pt is not None:
                        game_ctx["spreads"][name] = pt
                # If this is FanDuel and we found spreads, mark it as the context book
                if bkey == BOOKMAKER and game_ctx["spreads"]:
                    game_ctx["book_used"] = BOOKMAKER
            
            # totals
            elif mkey in {"totals", "total"} and game_ctx["game_total"] is None:
                for out in market.get("outcomes", []):
                    pt = out.get("point")
                    if pt is not None:
                        game_ctx["game_total"] = pt
                        break  # all outcomes in totals market share same point
                # If this is FanDuel and we found total, mark it
                if bkey == BOOKMAKER and game_ctx["game_total"] is not None:
                    game_ctx["book_used"] = BOOKMAKER
            
            # h2h / moneyline
            elif mkey in {"h2h", "moneyline"} and not game_ctx["moneyline"]:
                for out in market.get("outcomes", []):
                    name = out.get("name")
                    price = out.get("price")
                    if name is not None and price is not None:
                        game_ctx["moneyline"][name] = price
                # If this is FanDuel and we found moneyline, mark it
                if bkey == BOOKMAKER and game_ctx["moneyline"]:
                    game_ctx["book_used"] = BOOKMAKER
    
    # Set default book_used if still None
    if game_ctx["book_used"] is None and event.get("bookmakers"):
        game_ctx["book_used"] = event["bookmakers"][0].get("key")
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Backfill FanDuel NFL passing yards props from existing event dataset"
    )
    parser.add_argument(
        "--seasons",
        nargs="+",
...
def main(seasons=None, refresh_events=False):
    ...
    archive_file = ARCHIVE_DIR / f"fanduel_pass_yds_history_{'_'.join(map(str, sorted(seasons)))}_v1.csv"
